...
- 에이전트가 **팬레터의 내용과 온체인 후원 이력을 자율적으로 종합 판단**하여 다르게 행동한다:
  - **VIP 팬 (핵심 차별화)**: Nansen API를 통해 에이전트 지갑으로 과거 자산(ETH/USDC 등)을 후원한 이력이 확인되면 → **최우수 후원자 전용 프롬프트 (더 긴 감동적 답장)** 적용 + **Golden Reply NFT** 민팅
...
> 이것은 단순 텍스트 기반 챗봇이 아니다. 자신에게 온체인(On-chain)으로 기여한 팬을 오프체인(Off-chain) 소통에서 우대할 줄 아는 **경제적 지능을 갖춘 자율 에이전트**다.
...
**Agent Intelligence 시연 시나리오 (Nansen & AgentKit 연동):**
1. 심사위원이 일반 팬레터 발송 → 표준 답장 및 표준 Reply NFT 민팅
2. 사전에 에이전트 지갑으로 ETH를 소액 후원해둔 심사위원 지갑 주소로 팬레터 발송 → 에이전트가 Nansen API로 후원 이력 인식 → **VIP 감동 답장 생성 및 Golden Reply NFT** 민팅
...
- **VIP 팬 동적 등급 부여 (Core BM)**: On-chain 기여도(결제/후원 이력)가 Off-chain 소통(이메일 퀄리티)과 즉각적으로 연동되어, 팬들의 지속적인 후원과 결제를 유도하는 자생적 경제 생태계 구축.
- 플랫폼 수익: 에이전트 배포 및 관리 구독료
- 에이전트 지출: Flock.io 추론 비용을 x402로 자율 온체인 결제
- 에이전트는 "자율적으로 수익을 인식하고 운영 비용을 집행하는 완전한 경제 주체"
